...
export function rail(panel, api) {
  ...
  panel.group({ key: 'geometry', title: 'Radiator geometry', fields: [
    ...
    { key: 'traceW', type: 'range', label: 'Radiator trace width', unit: 'mm', min: 0.1, max: 3, step: 0.05, when: c => !patch(c) },
  ...
  panel.group({ key: 'feed', title: 'Feed and ground', fields: [
    { key: 'feedZ', type: 'range', label: 'Feed line impedance', unit: 'Ω', min: 20, max: 150, step: 1, when: patch },
    { key: 'feedLength', type: 'range', label: 'Feed length', unit: 'mm', min: 1, max: 100, step: 0.1, when: patch },
  ...
  panel.group({ key: 'substrate', title: 'Substrate', fields: [
    { key: 'boardT', type: 'range', label: 'Patch to ground spacing', unit: 'mm', min: 0.1, max: 3.2, step: 0.01, when: patch },
